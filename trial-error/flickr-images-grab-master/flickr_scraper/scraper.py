...
def get_image_list(clean_query,urls_api_key,urls_reqID,total_pages):
	...
	for i in range(1,2): #change range(1,total_pages) for all images , flickr loads only 4000 images on its server for any given search so ideal is range (1,9)
		page_number = str(i)
		...
		photos_data = (list(find('url_sq', response))) #replace url_sq with url_l for high def images
		
		for img_file in photos_data:
			img_name_regex = '_(.*?)_s.jpg'
